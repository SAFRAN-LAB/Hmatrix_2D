...
void HODLR_Node::printNodeDetails(int N,int n_levels)
{
        ...
        std::string fname;
        fname = "Output/hodlr_" + std::to_string(N) + "_" + std::to_string(n_levels) + ".txt";
        std::ofstream fout;
        fout.open(fname,std::ios::app);
        fout << "Level Number       :" << level_number << std::endl;
        fout << "Node Number        :" << node_number << std::endl;
        fout << "Start of Node      :" << n_start <<  std::endl;
        fout << "Size of Node       :" << n_size <<  std::endl;
        ...
        for(int i = 0; i < 2; i++)
        {
                ...
                fout << "Start of Child Node:" << c_start[i] <<  std::endl;
                fout << "Size of Child Node :" << c_size[i]  <<  std::endl;
        }

        fout << "Shape of U[0]      :" << U[0].rows() << ", " << U[0].cols() << std::endl;
        fout << "Shape of U[1]      :" << U[1].rows() << ", " << U[1].cols() << std::endl;
        fout << "Shape of V[0]      :" << V[0].rows() << ", " << V[0].cols() << std::endl;
        fout << "Shape of V[1]      :" << V[1].rows() << ", " << V[1].cols() << std::endl;
        fout << "Shape of K         :" << K.rows() << ", " << K.cols() << std::endl;
        fout.close();
}
